...
def train_gan():
    ...
    data_train_loader = DataLoader(data_train, batch_size=100, shuffle=True, num_workers=16)
    model = UNet(5, 1).cuda()
...
def validate_gan():
    ...
    train_transforms = transforms.Compose(train_transforms_list)
    data_train = ImageDataset(DeblurDataset(data_path).get_paths().data_train, transform=train_transforms)
    data_train_loader = DataLoader(data_train, batch_size=1, shuffle=False, num_workers=16)
